...
启用受控代理时，可通过 `VITE_EXHIBITION_REQUEST_TIMEOUT_MS` 缩短浏览器操作请求时限；远程模式默认且最高使用 `12000` 毫秒。服务端对单次飞书上游调用使用最高 8 秒时限，单个只读表读取包含最多一次重试且总预算最高为 10 秒；首屏共享读取默认使用最多 8 路受控并发。不要在浏览器配置或输出飞书令牌、Cookie、应用密钥或表标识。
