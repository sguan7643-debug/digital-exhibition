...
assert.match(applySource, /form\.type !== ['"]T003['"]/);
assert.match(applySource, /fetch\(['"]http:\/\/10\.151\.23\.119:28080\/api\/processInstanceStart['"],\s*\{/);
assert.match(applySource, /method:\s*['"]POST['"]/);
assert.match(applySource, /const formData = new FormData\(\)/);
assert.match(applySource, /formData\.append\(['"]request['"],\s*JSON\.stringify\(buildRpaApprovalRequest\(\)\)\)/);
assert.match(applySource, /formData\.append\(['"]file['"],\s*file,\s*file\.name\)/);
assert.match(applySource, /body:\s*formData/);
assert.doesNotMatch(applySource, /headers:\s*\{\s*['"]Content-Type['"]:\s*['"]application\/json['"]\s*\}/);
assert.match(applySource, /result\.code !== ['"]00000['"]/);
