...
for(const file of ['ToolDetailPage.vue','HainengWorkDetailPage.vue','ReportDetailPage.vue','DashboardDetailPage.vue']){
  ...
  const favorite=nodes.find(node=>node.type==='button'&&['收藏','已收藏'].includes(textOf(node)));
  ...
  nodes.find(node=>node.type==='button'&&textOf(node)==='申请使用').props.onClick();await nextTick();assert.match(textOf(live),/申请使用已在本地演示中登记/);
