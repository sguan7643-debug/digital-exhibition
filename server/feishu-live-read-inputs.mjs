...
function firstFileToken(value) {
  ...
  return typeof file === 'string' ? file : firstField(file, ['file_token', 'fileToken', 'token']);
}
...
export function createLiveReadInputResolver({ client, identifierContract, identity = {} } = {}) {
  if (!client?.listRecords || !identifierContract?.byName) throw new Error('读取输入解析器缺少客户端或标识契约');
  const cache = new Map();
  const readFirst = async tableName => {
  ...
  const ensureContext = async () => {
    ...
    const [app, announcement, course, certification, material, exportTask, archiveTask, connection, type, application] = await Promise.all([
      readFirst('应用索引'), readFirst('公告通知'), readFirst('培训课程'), readFirst('认证项目'),
      readFirst('素材中心'), readFirst('导出任务'), readFirst('归档任务'), readFirst('多维表连接配置'),
      readFirst('应用类型配置'), readFirst('使用申请')
    ]);
    ...
    context.materialId = firstField(materialFields, ['素材ID', '主键']) || text(material?.record_id);
    context.fileId = firstFileToken(materialFields['素材文件']) || firstField(materialFields, ['文件ID', '附件ID']);
  ...
  return Object.freeze({
    async resolve(operation) {
      ...
      if (needsContext.length) {
        const values = await ensureContext();
...
export { CONTEXT_REQUIREMENTS, STATIC_INPUTS };
