...
const sidebarExpanded = computed(() => shellState.expanded);
...
function toggleSidebar() { shellState.toggle(); }
...
<template>
  <div class="exhibition-shell" :class="{ 'standard-shell': !isCatalogue, 'certification-shell': props.page.id === '27' }">
    ...
    <div class="page-frame" :class="{ 'sidebar-collapsed': !sidebarExpanded }">
      <aside class="sidebar" :aria-label="sidebarExpanded ? '左侧导航（展开）' : '左侧导航（收起）'">
        <button class="sidebar-toggle" type="button" :aria-expanded="String(sidebarExpanded)" aria-controls="sidebar-content" @click="toggleSidebar">
        ...
        <template v-if="isCatalogue">
          <section class="catalogue-group">
            <h2><img src="/assets/catalogue-materials.png" width="24" height="24" alt="" />素材中心</h2>
            <nav aria-label="素材分类">
          ...
          <section class="catalogue-group app-group">
            <h2><img src="/assets/catalogue-apps.png" width="24" height="24" alt="" />应用中心</h2>
            <nav aria-label="应用分类">
