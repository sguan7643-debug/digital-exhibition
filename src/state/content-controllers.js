...
export const MESSAGE_FIXTURES = Object.freeze([
  { id:'message-001', type:'公告通知', title:'【新应用上线】供应商风险预警应用已发布上线', copy:'供应商风险预警应用正式发布上线，支持多维风险监测与预警。', time:'05-08 09:32', read:false, today:true, action:'查看详情' },
  { id:'message-002', type:'新应用上线', title:'【新应用上线】采购合同执行分析看板V2.0版本发布', copy:'采购合同执行分析看板V2.0版本已发布，优化采购合同可视化分析能力。', time:'05-07 16:18', read:false, today:true, action:'查看详情' },
  { id:'message-003', type:'申请进度', title:'应用上架申请已通过审批', copy:'库存周转分析报表已通过平台审核。', time:'05-07 11:05', read:false, today:true, action:'进入相关页面' },
  { id:'message-004', type:'培训课堂', title:'您已成功参加“数说心智·数智应用案例分享”', copy:'课程已加入您的学习计划。', time:'05-05 17:42', read:true, today:true, action:'进入课堂' },
  { id:'message-005', type:'导出完成', title:'数据导出任务已完成', copy:'本地演示不提供真实文件下载。', time:'05-05 15:33', read:true, today:true, action:'下载文件' },
  { id:'message-006', type:'平台通知', title:'平台将于5月10日22:00~23:00进行系统维护', copy:'维护期间部分演示功能受影响。', time:'05-04 14:21', read:true, today:true, action:'查看详情' }
].concat(Array.from({length:122},(_,index)=>{const number=index+7;const types=['公告通知','新应用上线','申请进度','培训课堂','平台通知'];return {id:`message-${String(number).padStart(3,'0')}`,type:types[index%types.length],title:`演示消息 ${String(number).padStart(3,'0')}`,copy:`固定种子817的第 ${number} 条本地消息。`,time:`04-${String(30-(index%28)).padStart(2,'0')} 09:00`,read:number>21,today:number<=9,action:'查看'};})).map(Object.freeze));
...
export function createMessagesController(fixtures) {
  ...
  return reactive({
    ...
    actionFor(item) {
      if(item.id==='message-001')return {kind:'route',route:'/announcements/notice-001'};
      if(item.type==='培训课堂')return {kind:'route',route:'/training'};
      if(item.type==='申请进度')return {kind:'route',route:'/apps/onboarding/status'};
      if(item.type==='新应用上线')return {kind:'route',route:'/apps'};
      return {kind:'feedback',message:item.type==='导出完成'?'演示环境不提供真实下载':`${item.title}：当前演示未提供独立详情页`};
    },
    activate(item){this.markRead(item.id);const action=this.actionFor(item);if(action.kind==='feedback')this.announcement=action.message;return action;},
...
export function createFavoritesController(fixtures, sharedFavorites=null) {
  return reactive({
    ...
    get activeCount(){return this.fixtures.filter(item=>!this.removed.includes(item.id)&&(!sharedFavorites||sharedFavorites.has(item.route))).length;},
    get results(){ const q=normalize(this.filters.query); return this.fixtures.filter(item=>!this.removed.includes(item.id)&&(!sharedFavorites||sharedFavorites.has(item.route))&&(!q||normalize(`${item.name} ${item.description}`).includes(q))&&(!this.filters.type||item.type===this.filters.type)&&(!this.filters.domain||item.domain===this.filters.domain)&&(!this.filters.tag||item.tag===this.filters.tag)); },
    get totalPages(){return Math.max(1,Math.ceil(this.results.length/this.pageSize));},
    get pagedResults(){const start=(this.page-1)*this.pageSize;return this.results.slice(start,start+this.pageSize);},
    ...
    cancel(id){const item=this.fixtures.find(card=>card.id===id);if(!this.removed.includes(id))this.removed.push(id);if(sharedFavorites&&item)sharedFavorites.delete(item.route);this.page=Math.min(this.page,this.totalPages);this.announcement=`已取消收藏，剩余 ${this.activeCount} 个`;},
    setPage(value){this.page=Math.min(this.totalPages,Math.max(1,Number(value)||1));this.announcement=`已切换到第 ${this.page} 页`;},
