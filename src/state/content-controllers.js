...
].map(([id,name,type,domain,description,usage,favorites,owner,developer,route], index) => Object.freeze({ id,name,type,domain,tag:domain,description,usage,favorites,owner,developer,route,image:`favorite-card-${index+1}.png` })));
...
export function createMessagesController(fixtures) {
  ...
  return reactive({
    ...
    setFilter(key,value) { this.filters[key]=value; this.announcement=`查询完成，共 ${this.results.length} 条消息`; },
    setStatus(value) { this.filters.status=value; this.announcement=`已切换消息状态，共 ${this.results.length} 条`; },
    ...
    refresh() { this.fixtures=originals.map(item=>({...item})); Object.assign(this.filters,{type:'',query:'',status:'all'}); this.sort='newest'; this.announcement='演示消息已刷新'; }
  });
}

export function createFavoritesController(fixtures) {
  return reactive({
    fixtures:[...fixtures], removed:[], filters:{query:'',type:'',domain:'',tag:''}, announcement:'',
    ...
    setFilter(key,value){this.filters[key]=value;this.announcement=`筛选完成，共 ${this.results.length} 个收藏`;},
    resetFilters(){Object.assign(this.filters,{query:'',type:'',domain:'',tag:''});this.announcement=`已清空筛选，共 ${this.results.length} 个收藏`;},
