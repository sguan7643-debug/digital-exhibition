    ...
    <section>
      ...
      <div class="progress-table horizontal-scroll-region" tabindex="0" role="region" aria-label="项目进度管理列表，可左右滚动">
        <table>
          ...
          <tbody><tr v-for="project in controller.pagedResults" :key="project.id"><td>{{ project.id }}</td><td>{{ project.name }}</td><td>{{ project.progress }}</td><td>{{ project.date }}</td><td>{{ project.scale }}</td><td>{{ project.score }}</td><td>{{ project.stage }}</td><td>{{ project.milestone }}</td><td v-for="label in PROGRESS_DOCUMENT_COLUMNS" :key="label"><button v-if="project.documents[label]===1" type="button" :aria-label="`${project.name} ${label}`" @click="controller.explainDocument(project,label)">文档</button><span v-else>{{ project.documents[label] }}</span></td><td><button type="button" :aria-label="`查看 ${project.name} 详情`" @click="controller.explainDetail(project)">查看详情</button></td></tr></tbody>
        </table>
      </div>
