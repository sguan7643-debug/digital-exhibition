...
function finishLoading() {
  ...
  loadingTimer = window.setTimeout(() => emit("restore"), 800);
}
function syncState(value) {
  window.clearTimeout(loadingTimer);
  localState.value = value;
  if (value === "loading") finishLoading();
}
function beginRetry() {
  localState.value = "loading";
  controller.announcement = "正在重新加载公告";
  finishLoading();
}
function applyAnnouncementFilters() {
  if (!controller.applyFilters()) nextTick(() => startDateInput.value?.focus());
}
  ...
  >
    ...
    <template v-if="contentVisible">
      ...
      <section class="notice-stats" aria-label="公告数据概览">
        <article>
          <img
            src="/assets/notice-stat-unread.png"
            width="67"
            height="67"
            alt=""
          />
        ...
        <article>
          <img
            src="/assets/notice-stat-new.png"
            width="67"
            height="67"
            alt=""
          />
      ...
      <form
        ...
        aria-label="公告筛选"
        @submit.prevent="applyAnnouncementFilters"
      >
        <label
          >公告类型：<select
            :value="controller.draft.type"
        ...
        >
        <label
          >发布时间：<span class="date-range"
            ><input
              ref="startDateInput"
              type="date"
              aria-label="开始日期"
              :aria-invalid="Boolean(controller.validationError)"
              aria-describedby="notice-date-error"
              :value="controller.draft.startDate"
              :disabled="controlsDisabled"
              @input="
                controller.setDraft('startDate', $event.target.value)
              " /><b>~</b
            ><input
              type="date"
              aria-label="结束日期"
              :aria-invalid="Boolean(controller.validationError)"
              aria-describedby="notice-date-error"
              :value="controller.draft.endDate"
              :disabled="controlsDisabled"
              @input="
                controller.setDraft('endDate', $event.target.value)
              " /></span
          ><small id="notice-date-error" class="filter-error" role="alert">{{
        ...
        >
        <button type="submit" :disabled="controlsDisabled">筛选</button
        ><button
          type="button"
          :disabled="controlsDisabled"
          @click="markAllRead"
      ...
      <section class="notice-table">
        <div
          v-if="state === 'empty' || !pagedAnnouncements.length"
          class="notice-empty"
        >
          <h2>暂无符合条件的公告</h2>
          <p>请调整筛选条件后重试。</p>
          <button type="button" @click="resetEmpty">清除筛选并恢复</button>
        </div>
        <template v-else
          ><table>
            ...
            <tbody>
              <tr v-for="item in pagedAnnouncements" :key="item.id">
                <td>
                  <i :class="item.tone" aria-hidden="true"></i
                  ><mark :class="item.tone">{{ item.type }}</mark>
                  <div>
                ...
                <td>
                  <time :datetime="`${item.date}T${item.time.slice(6)}`">{{
                    item.time
                  }}</time>
                </td>
                <td>
                ...
                <td>
                  <a
                    ...
                    href="/announcements/notice-001"
                    :aria-disabled="controlsDisabled"
                    @click="rememberDetail(item, $event)"
                    >查看详情　›</a
                  ><button
                    v-else
                    :id="`announcement-${item.id}`"
    ...
    <section
      v-else-if="state === 'permission-denied'"
      class="notice-state"
      role="alert"
    >
      <h1>访问受限</h1>
