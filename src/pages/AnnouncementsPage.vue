...
<template>
  <div class="announcements-page" :data-reference-sha="REFERENCE_SHA256" :data-state="state" :aria-busy="state === 'loading'">
    ...
    <template v-if="contentVisible">
      ...
      <section class="notice-stats" aria-label="公告数据概览"><article><img src="/assets/notice-stat-unread.png" width="67" height="67" alt="" /><div><strong>未读公告</strong><b>{{ controller.unreadCount }}</b><small>较昨日　<em class="down">↓ 5</em></small></div></article><article><img src="/assets/notice-stat-new.png" width="67" height="67" alt="" /><div><strong>本周新增</strong><b>9</b><small>较上周　<em>↑ 3</em></small></div></article></section>
...
<style scoped>
.announcements-page{min-height:calc(100vh - 79px);padding:27px 26px 15px;color:#10284a}.announcements-page>header{height:83px}.announcements-page h1{margin:0 0 7px;font-size:25px}.announcements-page header p{font-size:13px}.notice-stats{display:grid;grid-template-columns:repeat(2,1fr);gap:17px}.notice-stats article{height:125px;display:flex;align-items:center;justify-content:center;gap:28px;background:#fff;border:1px solid #d9e3ed;border-radius:5px}.notice-stats img{width:67px;height:67px}.notice-stats div{display:grid;gap:5px}.notice-stats strong{font-size:12px}.notice-stats b{font-size:27px}.notice-stats small{font-size:11px}.notice-stats em{color:#079347;font-style:normal}.notice-stats em.down{color:#e22}.notice-filters{min-height:75px;display:flex;align-items:center;gap:16px;padding:10px 24px;margin:13px 0;background:#fff;border:1px solid #d9e3ed;border-radius:5px;font-size:12px}.notice-filters label{position:relative;display:flex;align-items:center;gap:10px;white-space:nowrap}.notice-filters select,.notice-filters input{height:38px;border:1px solid #d1ddea;border-radius:4px;background:#fff;padding:0 12px}.notice-filters select{width:150px}.date-range{display:flex;align-items:center;width:272px;height:38px;border:1px solid #d1ddea;border-radius:4px}.date-range input{width:120px;border:0}.date-range b{font-weight:400}.filter-error{position:absolute;left:68px;top:41px;color:#c62828}.notice-filters button{height:39px;padding:0 18px;border:0;color:#fff;background:#075fd2;border-radius:4px}.notice-filters button:disabled{color:#8996a6;background:#e7ebf0}.notice-table{background:#fff;border:1px solid #d9e3ed;border-radius:5px}table{width:100%;border-collapse:collapse}th{height:50px;color:#28405f;font-size:11px;text-align:left}th:first-child{padding-left:24px}th:nth-child(2){width:145px}th:nth-child(3){width:115px}th:nth-child(4){width:120px}td{height:65px;border-top:1px solid #e1e8ef;font-size:11px}td:first-child{display:grid;grid-template-columns:12px 80px minmax(0,1fr);align-items:center;gap:7px;padding:8px 24px}td i{width:6px;height:6px;border-radius:50%}td i.red{background:#e43c3c}td i.green{background:#07944a}td i.orange{background:#ff6d00}td i.purple{background:#7333dd}td i.blue{background:#0870ed}td mark{justify-self:start;padding:3px 8px;border-radius:3px;font-size:9px}td mark.red{color:#e23a3a;background:#ffeaea}td mark.green{color:#0a9149;background:#e6f8ee}td mark.orange{color:#f16400;background:#fff0e5}td mark.purple{color:#6d2ee0;background:#f0e9ff}td mark.blue{color:#0870e8;background:#e7f2ff}td strong{display:block;font-size:12px}td p{margin-top:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#526985;font-size:10px}td time{color:#34506f}td>span{color:#5f7086}td>span.unread{color:#0870e8}td>span.unread::before{content:'';display:inline-block;width:6px;height:6px;margin-right:7px;background:#0870e8;border-radius:50%}td a,td button{color:#0870e8;background:none;border:0;font:inherit}.notice-table footer{height:65px;display:flex;align-items:center;gap:20px;padding:0 19px;font-size:11px}.notice-table footer>span{margin-right:auto}.notice-table footer select,.notice-table footer input{height:34px;border:1px solid #d3dfea;background:#fff}.notice-table footer nav{display:flex;gap:7px}.notice-table footer button{width:31px;height:31px;border:1px solid #d3dfea;background:#fff;border-radius:4px}.notice-table footer button[aria-current=page]{color:#fff;background:#0868dc}.notice-table footer input{width:40px;text-align:center}.notice-empty,.notice-state{min-height:300px;display:grid;place-content:center;justify-items:center;text-align:center}.notice-empty h2,.notice-state h1{margin:0 0 8px;font-size:20px}.notice-empty p,.notice-state p{color:#66798f}.notice-empty button,.notice-state button,.notice-state a{min-height:38px;padding:0 18px;display:inline-grid;place-items:center;color:#fff;background:#075fd2;border:0;border-radius:4px}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}:focus-visible{outline:3px solid #ff9f1a;outline-offset:2px}@media(max-width:1100px){.notice-filters{height:auto;flex-wrap:wrap;padding-block:12px}.notice-table{overflow-x:auto}.notice-table table,.notice-table footer{min-width:1050px}}@media(max-width:760px){.announcements-page{padding:12px}.notice-stats{grid-template-columns:1fr}}
.announcements-page{padding:47px 26px 15px 39px}.announcements-page>header{height:78px}
</style>
