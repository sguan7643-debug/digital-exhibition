...
const rules = computed(() => {
  ...
  return Array.isArray(items) && items.length ? items.map((rule) => ({
    name: rule.ruleName || rule.sourceName || rule.ruleId,
    points: rule.points,
    frequency: rule.frequencyType || '按规则',
    description: rule.description || `${rule.sourceName || '该行为'}按规则累计`,
  })) : [
    { name: '应用使用积分', points: 5, frequency: '每日', description: '每日首次使用应用可获得积分' },
    { name: '培训学习积分', points: 10, frequency: '每次', description: '完成培训学习后按规则累计' },
  ];
});
