...
<template>
  <article class="apps-page" aria-labelledby="apps-title">
    ...
    <header>
      ...
      <a class="onboarding-link" href="/apps/onboarding/apply">应用上架申请</a>
    </header>
    ...
    >
      <article v-for="app in pagedApps" :key="app.id" :data-app-id="app.id" class="application-card" @click="openCard">
        ...
        <footer>
          ...
          <a v-if="applicationAccessMode(app.route) === 'direct'" class="card-access" :href="`${app.route}#usage`">立即使用</a>
          <button v-else-if="applicationAccessMode(app.route) === 'apply'" class="card-access" type="button" @click="localAction('申请使用', app)">申请使用</button>
