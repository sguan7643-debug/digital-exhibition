...
const filteredApps = computed(() => controller.results.filter(app => app.id !== 'app-report-001').map(displayApp));
...
<template>
  <article class="apps-page" data-visual-baseline="ui-update-0831-apps" aria-labelledby="apps-title">
    ...
    <header><div><h1 id="apps-title">应用中心</h1><p>汇聚优质应用资源，助力业务高效协同与智能决策</p></div><button type="button" @click="controller.announcement='应用上线申请为本地演示操作'">应用上线申请</button></header>
    ...
    <div class="apps-tools"><strong>全部应用 {{ filteredApps.length }} 个</strong><button type="button" @click="resetFilters">清空筛选</button><span v-if="filter.category" class="active-category">{{ filter.category }}</span><select :value="controller.sort" aria-label="排序" @change="controller.setSort($event.target.value)"><option value="default">综合排序</option><option value="usage-desc">使用量从高到低</option><option value="favorites-desc">收藏量从高到低</option><option value="name">名称排序</option></select><button type="button" aria-label="卡片视图" :aria-pressed="controller.view === 'grid'" @click="controller.setView('grid')"><img src="/assets/catalogue-apps.png" width="16" height="16" alt="" /></button><button type="button" aria-label="列表视图" :aria-pressed="controller.view === 'list'" @click="controller.setView('list')"><img src="/assets/nav-materials.png" width="16" height="16" alt="" /></button></div>
    <section v-if="filteredApps.length" class="apps-grid" :class="{ 'list-view': controller.view === 'list' }" aria-label="应用列表"><article v-for="app in pagedApps" :key="app.id"><header><img :src="`/assets/${app.image}`" width="52" height="52" alt="" /><div><h2><mark>{{ app.tag }}</mark>{{ app.name }}</h2><p><mark>{{ app.category }}</mark><mark>{{ app.scene }}</mark></p></div></header><p>{{ app.description }}</p><dl><div><dt>使用量</dt><dd>{{ app.usage.toLocaleString('zh-CN') }}</dd></div><div><dt>收藏</dt><dd>{{ app.favorites.toLocaleString('zh-CN') }}</dd></div><div><dt>负责人</dt><dd>{{ app.owner }}</dd></div><div><dt>开发者</dt><dd>{{ app.developer }}</dd></div></dl><footer><a :id="`app-${app.id}`" :data-session-focus="`app-${app.id}`" :href="app.route">查看详情</a><button type="button" @click="localAction('立即使用', app)">立即使用</button><button type="button" :aria-pressed="routeSession.isRouteFavorite(app.route)" @click="toggleFavorite(app.route)">{{ routeSession.isRouteFavorite(app.route) ? '已收藏' : '收藏' }}</button><button type="button" @click="localAction('申请试用', app)">申请试用</button></footer></article></section>
    ...
    <PaginationControl class="apps-pagination" :total="filteredApps.length" :page="controller.page" :page-size="controller.pageSize" :page-sizes="[12]" label="应用中心分页" @update:page="controller.setPage" @update:page-size="controller.setPageSize" />
  </article>
</template>
