...
export function createDataState({ data = null, mode = 'mock', state } = {}) {
  return Object.freeze({
    ...
    dataStale: false, lastSyncedAt: null, unavailableSections: [], unavailableReasonCode: null, retryScope: []
  });
}
export function reduceDataState(previous, event) {
  ...
  if (event.type === 'success') {
    ...
    return Object.freeze({
      ...previous, state: event.state || (empty ? 'empty' : 'normal'), data: event.data, error: null,
      ...
      retryScope: event.retryScope || [], announcement: empty ? '没有符合条件的数据' : '数据已更新'
    });
  }
  ...
  if (event.type === 'fail') {
    ...
    return Object.freeze({
      ...
      announcement: state === 'cancelled' ? '' : '数据更新失败'
    });
  }
  return previous;
}
